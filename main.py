...
【网页只读面板】插件配置 web_server_port 为本机端口后，发 /watcher web 仅返回浏览器链接"""
...
上次检查: {cfg.get('last_check', '从未')}"""
    ...
    async def cmd_organize(self, event: AstrMessageEvent):
        ...
        try:
            organized = strip_fenced_markdown(
                await self._call_ai(
                    prompts.TASKBOOK_ORGANIZE_PROMPT.format(
                        content=cfg["taskbook_content"]
                    )
                )
            )
    ...
    async def cmd_check(self, event: AstrMessageEvent):
        ...
        try:
            ...
            task_count = max(
                1, min(30, len(re.findall(r"^\s*##\s+\d+\.", tb, re.MULTILINE)) or 1)
            )
            # AI 分析任务完成度
    ...
    async def cmd_status(self, event: AstrMessageEvent):
        ...
        n_numbered = len(re.findall(r"^\s*##\s+\d+\.", content, re.MULTILINE))
        if n_numbered:
            text = f"""📊 任务状态（固定格式）
编号条目数: {n_numbered}
上次检查: {cfg.get("last_check", "从未")}
